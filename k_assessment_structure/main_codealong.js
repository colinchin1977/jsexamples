...
const _WIDTHPROMPT = `Select number of tiles for the WIDTH (min. ${_MIN} - max. ${_MAX}): `;
const _HEIGHTPROMPT = `Select number of tiles for the HEIGHT (min. ${_MIN} - max. ${_MAX}): `;
const _INVALID = `Please enter a valid number between ${_MIN} - ${_MAX}.\n`;

// DONE: Set the constants to display _WIN, _LOSE or _OUTOFBOUNDS
const _OUTOFBOUNDS = "Out of Bounds - You lose!";
const _WIN = "Congratulations! You found your hat!";
const _LOSE = "You fell into a hole - You lose!";
const _QUIT = "Goodbye! Come back soon!";
...
class Field {
    ...
    startGame() {
        ...
        while(this.playGame) {
            ...
            // DONE: Prompt the player to move
            this.promptMove();
            
            // DONE: Track the player's position
            this.trackPlayer();
    ...
    printInstructions() {
        console.log('To move the character, use WASD commands - Enter:\n> "w" to move UP\n> "a" to move LEFT\n> "s" to move DOWN\n> "d" to move RIGHT\n> Or enter "q" or CTRL + C to quit.\n')
    }
    ...
    promptMove() {
        ...
        // DONE: Move base on input for moveDirection (inform the player if it is an invalid move or the game has ended via "q")
        switch (moveDirection) {
            ...
            case "q":
                ...
                break;
            default:
                console.log("Invalid input - Please use WASD commands, or enter q or Ctrl+C to quit");
                this.promptMove;
        }
    }      
    ...
    trackPlayer() {
        ...
        // DONE: Setting the condition to check if the player has gone off-bounds, fallen into a hole or found the hat
        
        switch (true) {
                case this.positionY < 0 || this.positionX < 0 || this.positionY > this.field.length - 1 || this.positionX > this.field.length - 1:
                    this.endGame(_OUTOFBOUNDS);
                    break;
    ...
    endGame(msg) {
        // DONE: End the game using the message passed in
        this.playGame = false;
        console.log(msg);
    ...
    static createField(width, height) {
        ...
        // Create a random number between 0.1 and 0.2, keep the values at 1 decimal place
        ...
        for (let y = 0; y < height; y++) {
            ...
            //  If the limit is less than the ceiling, fill with the fieldCharacter - otherwise, fill with th hole
        ...
        field[hatPosition.x][hatPosition.y] = hat;
...
// Setting the dimensions of the game board
const gameDimensions = Field.gameDimensions();

// Create a 2D array of the game's field using gameDimensions's width and height
